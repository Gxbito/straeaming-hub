...
document.getElementById('homeForm').addEventListener('submit', function (event) {
  ...
  const formData = {
    firstName: document.getElementById('homeFirstName').value,
    lastName: document.getElementById('homeLastName').value,
    email: document.getElementById('homeEmail').value,
    phoneNumber: document.getElementById('homePhoneNumber').value,
    postalCode: document.getElementById('homePostalCode').value,
    platformPreference: document.getElementById('homePlatformPreference').value
  };

  // Envía los datos a HubSpot
  var xhr = new XMLHttpRequest();
  var url = 'https://api.hsforms.com/submissions/v3/integration/submit/portalId/homeFormId'; // Reemplaza con tu URL de HubSpot
